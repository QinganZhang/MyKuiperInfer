...
namespace kuiper_infer
{
    ...
    {
        CHECK(shapes.size() == 3); // shapes must be three dimensions
        ...
        const std::shared_ptr<Tensor<float>> &tensor2)
    {
        ...
        if (tensor1->shapes() == tensor2->shapes())
        {
            f_tensor_sptr output_tensor = TensorCreate(tensor1->shapes());
            output_tensor->set_data(tensor1->data() + tensor2->data());
        ...
        else
        {
            ...
            f_tensor_sptr output_tensor = TensorCreate(input_tensor1->shapes());
            output_tensor->set_data(input_tensor1->data() + input_tensor2->data());
            return output_tensor;
        }
    }
        ...
        const std::vector<uint32_t> &pads, float padding_value)
    {
        ...
        std::shared_ptr<f_tensor> output = std::make_shared<f_tensor>(
    ...
    std::tuple<f_tensor_sptr, f_tensor_sptr> TensorBroadcast(const f_tensor_sptr &s1,
                                                             const f_tensor_sptr &s2)
    {
