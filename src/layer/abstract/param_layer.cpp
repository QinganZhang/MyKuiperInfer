...
void ParamLayer::InitBiasParam(const uint32_t param_count, const uint32_t param_channel,
    ...
    this->bias_ = std::vector<f_tensor_sptr>(param_count);
    for (uint32_t i = 0; i < param_count; ++i) {
        this->bias_.at(i) = std::make_shared<f_tensor>(param_channel, param_height, param_width);
    }
}

void ParamLayer::InitWeightParam(const uint32_t param_count, const uint32_t param_channel,
    const uint32_t param_height, const uint32_t param_width) {
    this->weights_ = std::vector<f_tensor_sptr>(param_count);
    for (uint32_t i = 0; i < param_count; ++i) {
        this->weights_.at(i) = std::make_shared<f_tensor>(param_channel, param_height, param_width);
    }
}

const std::vector<std::shared_ptr<Tensor<float>>>& ParamLayer::weights() const { return this->weights_; }
